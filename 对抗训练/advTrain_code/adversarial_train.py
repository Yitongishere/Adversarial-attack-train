...
from fmnist_dataset import FashionMNISTDataset
...
def trainEpochs(classifier, optimizer, loss_fn, epochs, training_set, dev_set,
    ...
    for ep in range(1, epochs + 1):
        ...
        torch.save(classifier.state_dict(),
                   os.path.join(save_dir, 'Adversarial_train_ep_' + str(ep) + '_devacc_' + str(acc) + '_.pt'))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_dir', type=str, default='../advTrain_model')
    ...
    # original dataset
    train, dev, _ = load_fashion_mnist(opt.dataset_dir, random=random)

    # adversarial data
    file = open("../../白盒攻击/whitebox_attack_data/adversarial_samples_1k.pkl", 'rb')
    adv_data = pickle.load(file)

    # Concat original data and adversarial data
    train_adv = FashionMNISTDataset(adv_data)
    train_new = ConcatDataset([train, train_adv])

    # shuffle the new training set and load data
    train_dataloader = DataLoader(train_new, batch_size=opt.batch_size, shuffle=True, drop_last=True)
